...
class Bests:
  ...
  def update(self, sigs, bkgs, nons, sigmas):
    ...
    for i in range(self.nCats):
      ...
      self.Signifs.append(signif)
      print self.Signifs
      totSignifSq += signif*signif
    totSignif = np.sqrt( totSignifSq )
    return Ture
    '''
    if (totSignif > self.totSignif) and (tags_ok):
...
class CatOptim:
  ...
  def optimise(self, lumi, nIters):
    ...
    cuts = od()
    for iName,name in enumerate(self.names):
      cutslist = np.linspace(self.lows[name], self.highs[name],70)
      for i in cutslist:
        tempCuts = i
        cuts[name] = tempCuts
      sigs   = []
  ...
  def getPrintableResult(self):
    ...
    printStr += '\n'
    printStr += 'the significance are %s'%self.bests.Signifs
    return printStr
