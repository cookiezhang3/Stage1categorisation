...
#for loop fpr optimisation
#for i, j, k in itertools.product(np.arange(100,200,50),np.arange(100,200,50),np.arange(100,250,50)):
for i in [1]:
  #print 'i,j,k', (i,j,k)
  #build the model
  model=set_model(newVars,dijetVars,n_lstm_layers=2, n_lstm_nodes=100, n_dense_1=3, n_nodes_dense_1=100, 
                       n_dense_2=2, n_nodes_dense_2=200, dropout_rate=0.25,
                       learning_rate=0.002, batch_norm=True, batch_momentum=0.99)
...
model.save('%s/LSTM.model'%(modelDir))
print 'saved as %s/LSTM.model'%(modelDir)

#plot ROC
plotDir = trainDir.replace('trees','plots_LSTM')
...
plt.plot(fpr_tr,tpr_tr,label = r'training set ROC curve (area = 0.927 $\pm$ 0.002)')
plt.plot(fpr_tst,tpr_tst,label = r'test set ROC curve (area = 0.926 $\pm$ 0.001)')
