...
from LSTM_func import join_objects
...
#define the variables used as input to the classifier
    ...
    dataN  = dataTotal[newVars_flat].values
...
#create 2D objects from low level variables
    vbfN_2D = join_objects(vbfN,newVars_flat,newVars)
    dataN_2D = join_objects(dataN,newVars_flat,newVars)
    numOutputs = 3
...
#load and evaluate VBF three class
  ...
  #load model
    model = keras.models.load_model('%s/%s'%(modelDir,'LSTM.model'))
    vbfPredictions = model.predict([vbfX,vbfN_2D], batch_size=batchSize).reshape(vbfM.shape[0],numOutputs)
    ...
    dataPredictions = model.predict([dataX,dataN_2D]).reshape(dataM.shape[0],numOutputs)
